...
class SemanticSearchGUI:
    ...
    def index_text_files(self, progress=gr.Progress()) -> str:
        ...
        try:
            ...
            result = f"✅ **Text Indexing Complete** ({stats.get('time', 0):.2f}s)\n\n"
    ...
    def index_images(self, progress=gr.Progress()) -> str:
        ...
        try:
            ...
            result = f"✅ **Image Indexing Complete** ({stats.get('time', 0):.2f}s)\n\n"
    ...
    def search_text(self, query: str, n_results: int) -> str:
        ...
        try:
            ...
            results = self.text_engine.search(
                query, n_results=int(n_results), directory_filter=self.directory
            )
            ...
            output = (
                f"🔍 **Search:** {query} ({elapsed:.2f}s, {len(results)} results)\n\n"
            )
            output += "---\n\n"
    ...
    def search_images_by_text(
        ...
        try:
            ...
            if stats["total_images"] == 0:
                return (
                    [],
                    "⚠️ No indexed images found. Please go to Management tab and click 'Index Images' first.",
                )
    ...
    def search_images_by_reference(
        self, ref_image: str, n_results: int
    ) -> Tuple[List[Tuple[str, str]], str]:
        ...
        try:
            ...
            results = self.image_engine.search_by_image(
                ref_image, n_results=int(n_results), directory_filter=self.directory
            )
            elapsed = time.time() - start_time
...
def create_gui():
    ...
    with gr.Blocks(
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=1):
                        text_n_results = gr.Slider(
                            minimum=1, maximum=20, value=5, step=1, label="Results"
                        )
            ...
            with gr.Tab("🖼️ Image Search - Reference"):
                ...
                with gr.Row():
                    with gr.Column(scale=2):
                        ref_image = gr.Image(
                            label="Reference Image", type="filepath", height=300
                        )
                    with gr.Column(scale=1):
                        ref_n_results = gr.Slider(
                            minimum=1, maximum=20, value=5, step=1, label="Results"
                        )
                        ref_search_btn = gr.Button(
                            "🔍 Find Similar", variant="primary", size="lg"
                        )
            ...
            with gr.Tab("⚙️ Management"):
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        text_index_btn = gr.Button(
                            "Index Text Files", variant="secondary", size="lg"
                        )
                        text_index_output = gr.Markdown("")

                    with gr.Column():
                        gr.Markdown("#### 🖼️ Images")
                        image_index_btn = gr.Button(
                            "Index Images", variant="secondary", size="lg"
                        )
        ...
        text_index_btn.click(fn=gui.index_text_files, outputs=[text_index_output])

        image_index_btn.click(fn=gui.index_images, outputs=[image_index_output])

        stats_btn.click(fn=gui.get_stats, outputs=[management_output])

        clear_text_btn.click(fn=gui.clear_text_db, outputs=[management_output])

        clear_image_btn.click(fn=gui.clear_image_db, outputs=[management_output])
